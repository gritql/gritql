...
function gaQueryBuilder(table, tree, queries = [], idx = undefined, knex, metricResolvers) {
    ...
    if (!!~idx && idx !== undefined && !queries[idx])
        queries[idx] = { idx, name: undefined, source: 'GA' };
    ...
    if (!!tree.selectionSet?.selections) {
        ...
        if (tree.name?.value !== 'fetch' && !tree.with)
    ...
    if (args?.groupBy) {
        query.postQueryTransform.push((result) => {
            return result.reduce((r, l) => {
                const newDate = luxon_1.DateTime.fromISO(l[tree.name.value]).startOf(args?.groupBy).toISODate();
